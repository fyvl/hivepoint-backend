...
type UsageServiceMock = {
    ...
    recordAuthorizedUsage: jest.Mock;
};
...
describe('GatewayService', () => {
    ...
    it('dispatches through the latest published version and records usage', async () => {
        ...
        usageService.recordAuthorizedUsage.mockResolvedValue(undefined);
        ...
        expect(usageService.recordAuthorizedUsage).toHaveBeenCalledWith(
            expect.objectContaining({
                subscriptionId: 'sub-1',
                endpoint: '/health',
                requestCount: 1,
    ...
    it('rejects dispatch when usage authorization denies access', async () => {
        ...
        expect(usageService.recordAuthorizedUsage).not.toHaveBeenCalled();
    });
});
