...
| `LLM_NOT_CONFIGURED`              | 503              | Seller Studio description generation when `LLM_MODEL` is not configured.                                                                                                                |
| `LLM_UPSTREAM_UNAVAILABLE`        | 502              | Seller Studio description generation when the configured LLM provider fails, times out, or returns no usable completion.                                                                |
| `ML_SUGGESTIONS_DISABLED`         | 503              | Seller Studio category/tag suggestions when `ML_SUGGESTIONS_ENABLED=false`.                                                                                                             |
| `ML_UPSTREAM_UNAVAILABLE`         | 502              | Seller Studio category/tag suggestions when the configured ML service fails, times out, or returns an invalid payload.                                                                  |
