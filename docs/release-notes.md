...
- Usage metering: authorize raw API keys against active subscriptions, quota, and optional per-plan RPM policy; accept queue-backed usage ingest events with async persistence into `UsageRecord`; maintain subscription-level daily aggregates; and summarize usage per active subscription.
