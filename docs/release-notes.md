...
- Billing: plans, subscriptions, invoices, mock payment endpoints, Stripe Checkout, webhook sync, Stripe customer portal, renewal state sync, app-managed renewal retries for `past_due` Stripe renewals, automated overage settlement/collection, zero-base Stripe pay-per-use renewals, grace period, retry metadata, and overage-enabled plan configuration.
...
- Usage metering: authorize raw API keys against active subscriptions, quota, optional overage policy, and optional per-plan RPM policy; accept queue-backed usage ingest events with async persistence into `UsageRecord`; maintain subscription-level and per-endpoint daily aggregates; and summarize usage per active subscription with projected overage charges plus top-endpoint breakdowns.
- Observability: every HTTP response includes `x-request-id`, errors surface `requestId`, requests are logged in a structured format, `GET /metrics` exposes Prometheus-style metrics, operational snapshots are persisted for history, `GET /admin/ops/dashboard` exposes an ops dashboard snapshot plus history, external operational alerts can fan out to multiple configured webhooks with reminder cooldown, and admin moderation actions are persisted in `AuditLog`.
...
- Usage read models now include subscription-level and per-endpoint daily aggregates, but richer historical filters and seller-facing drill-downs are not implemented yet.
- Role changes are done via database updates (no admin UI).
- Observability now includes persisted metrics history and multi-target webhook fan-out, but richer incident-provider integrations and deeper dashboard drill-downs are not implemented yet.
